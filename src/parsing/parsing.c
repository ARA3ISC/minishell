...
void checking_rediection_in_the_last(char *cmd)
{
	...
	while(cmd[i] == ' ')
		i--;
	if(cmd[i] == '>' || cmd[i] == '<')
		print_error("syntax error", 102); // 3aaaaaaabi viifie hna l exit status ach khaso ykon !!!!!!!!!!!!!!!!!!!
}
...
void	syntax_error(char *cmd)
{
	size_t i = 0;
	int check = 0;
	checking_rediection_in_the_last(cmd);
	while (cmd[i])
	{
